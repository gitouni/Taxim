...
def cropMap(deformMap):
    ...
    elif np.ndim(deformMap) ==2:
        croppedMap = deformMap[psp.d//2-psp.h//2:psp.d//2+psp.h//2,psp.d//2-psp.w//2:psp.d//2+psp.w//2]
    else:
        raise UserWarning("Dim of deformMap should be 2 or 3.")
    return croppedMap

def insertMap(deformMap):
    if np.ndim(deformMap) == 3:
        channel = deformMap.shape[0]
        insertedMap = np.zeros((channel,psp.d,psp.d))
        for c in range(channel):
            insertedMap[c,psp.d//2-psp.h//2:psp.d//2+psp.h//2,psp.d//2-psp.w//2:psp.d//2+psp.w//2] = deformMap
    elif np.ndim(deformMap) ==2:
        insertedMap = np.zeros((psp.d,psp.d))
        insertedMap[psp.d//2-psp.h//2:psp.d//2+psp.h//2,psp.d//2-psp.w//2:psp.d//2+psp.w//2] = deformMap
    else:
        raise UserWarning("Dim of deformMap should be 2 or 3.")
    return insertedMap
...
class SuperPosition:
    ...
    def compose_sparse(self, local_deform, gel_map, contact_mask):
        ...
        local_kx = psp.d//2
        local_ky = psp.d//2
        # sparse_mask = cropMap(self.sparse_mask)
    ...
    def compose_sparse_insert(self, local_deform, _gel_map, _contact_mask):
        """
        local deform: (3,1) dx, dy, dz initial displacement
        gel map: height map
        contact_mask: indicate contact points
        ...
        # sparse_mask = cropMap(self.sparse_mask)
        all_points = np.where(self.sparse_mask == 1)
        contact_mask = insertMap(_contact_mask)
        gel_map = insertMap(_gel_map)
        contact_points = np.where((contact_mask == 1) & (self.sparse_mask == 1))
        num_points = contact_points[0].shape[0]
