...
parser.add_argument("--radius", default=2.832, type=float, help="unit: mm")
parser.add_argument('--max_depth', default=2.832, type=float, help='Indetation depth into the gelpad.')
parser.add_argument('--min_depth',default= 0.5, type=float)
parser.add_argument("--max_margin_ratio",type=float,default=0.6)
parser.add_argument("--iter",type=int,default=100)
parser.add_argument("--seed",type=int,default=0)
parser.add_argument("--suffix",type=str,default=".pcd")
parser.add_argument("--zero_mean",type=bool,default=False)

args = parser.parse_args()

class simulator(object):
    def __init__(self, data_folder:str, radius:float):
        """construct the gelsight simulator

        Args:
            data_folder (str): the directory of the calibration npz
...
if __name__ == "__main__":
    ...
    os.makedirs(osp.join("..","results","asim"), exist_ok=True)
    os.makedirs(osp.join("..","results","ashadow"), exist_ok=True)
    os.makedirs(osp.join("..","results","aheight"), exist_ok=True)
    sim = simulator(data_folder, args.radius)
    ...
    img_savePath = osp.join('..', 'results', 'asim', '%04d.jpg'%(0))
    shadow_savePath = osp.join('..', 'results', 'ashadow', '%04d.jpg'%(0))
    height_savePath = osp.join('..', 'results', 'aheight', '%04d.npz'%(0))
    ...
    for i, (depth, du, dv) in tqdm(enumerate(zip(depth_list, du_list, dv_list)), total=len(du_list)):
        ...
        # simulate tactile images
        sim_img, shadow_sim_img, dzdx, dzdy = sim.simulating(heightMap, contact_mask, contact_height, shadow=True)
        img_savePath = osp.join('..', 'results', 'asim', '%04d.jpg'%(i+1))
        shadow_savePath = osp.join('..', 'results', 'ashadow', '%04d.jpg'%(i+1))
        height_savePath = osp.join('..', 'results', 'aheight', '%04d.npz'%(i+1))
        cv2.imwrite(img_savePath, sim_img)
        cv2.imwrite(shadow_savePath, shadow_sim_img)
        np.savez(height_savePath, height=height_map*psp.pixmm, dzdx=dzdx, dzdy=dzdy, contact_mask=contact_mask)
